...
class Line:
    ...
    def process_message(self, message):
        ...
        if "stations.table" in message.topic(): # Faust station transform data topic
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "arrivals" in message.topic(): # Arrival 
            self._handle_arrival(message)
        elif "TURNSTILE_SUMMARY" in message.topic(): # KSQL Turnstile Summary Topic
